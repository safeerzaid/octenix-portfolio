  ...
  return (
    ...
    >
      <ul
        ...
        className="m-0 flex list-none flex-col py-4 [gap:var(--item-gap)] w-full"
      >
        {items.map((label, index) => (
          <li
            ...
            onPointerEnter={() => handleClick(index, label)} 
            className={`relative cursor-pointer before:absolute before:-inset-x-12 before:-inset-y-[6px] before:content-[''] ${tickClass} transition-colors duration-300`}
          >
            {showMarker && (
              <span
                aria-hidden="true"
            ...
            <span className="relative inline-flex items-baseline leading-[1.2] [color:color-mix(in_srgb,var(--accent-color)_calc(var(--effect,0)*100%),var(--text-color))] [font-size:var(--font-size)] [transform:translateX(calc(var(--effect,0)*var(--max-shift)))] w-full">
              {showIndex && (
